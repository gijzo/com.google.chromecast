...
function ConnectionController(client, sourceId, destinationId) {
  console.log('CONSTRUCTOR ConnectionController');
  ...
  function onmessage(data, broadcast) {
    console.log('ON MESSAGE DATA TYPE', data.type);
    if(data.type === 'CLOSE') {
      connected = false;
  ...
  function onclose() {
    console.log('connection close', sourceId);
    self.removeListener('message', onmessage);
    if(connected) {
      console.log('connection force disconnect', sourceId);
      self.emit('disconnect');
    }
  }
...
ConnectionController.prototype.connect = function() {
  console.log('connect called', this.sourceId);
  this.emit('test');
