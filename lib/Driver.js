...
class Driver extends events.EventEmitter {
	...
	_initDevice(deviceData) {
		...
		if (!deviceData.id || deviceData.id.length !== 32) {
			return this.setUnavailable(deviceData, __('repair'));
		}

		this.setUnavailable(deviceData, __('unavailable'));

		// get device object
		let oldDevice = this.getDevice(deviceData);
		oldDevice = oldDevice instanceof Error ? {} : oldDevice;
		// get local device
		const mdnsDevice = this._mdnsDevices[deviceData.id];
		if (mdnsDevice) {
			...
			if (!oldDevice.speaker) {
				this._registerSpeaker(deviceData);
			}
		}
		this.on(`device:${deviceData.id}`, () => {
			this._initDevice(deviceData);
		});
	...
	_uninitDevice(deviceData) {
		...
		if (device.speaker) {
			...
			this.unregisterSpeaker(deviceData, (() => null));
		}
	}

	_registerSpeaker(deviceData) {
		this.registerSpeaker(deviceData, {
			codecs: ['homey:codec:mp3'],
		}, (err, speaker) => {
			if (err) return Homey.error(err);
			this._devices[deviceData.id].speaker = speaker;
	...
	castMediaUrl(device, audioUrl, callback) {
		this.log('castUrl');
	...
	castUrl(device, url, callback) {
		...
		this.getApplication(device, Browser).then((res) => {
			...
			browser.redirect(this.sanitizeUrl(url.trim()), (err, result) => {
				disconnect();
				callback(err, result);
			});
		}).catch(err => {
