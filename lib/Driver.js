...
class Driver extends events.EventEmitter {
		...
		if (mdnsDevice) {
			...
			this.registerSpeaker(device_data, {
				...
				speaker.on('setTrack', (track, callback) => {
					...
					this.getApplication(device, Media).then((player) => {
				...
				speaker.on('setPosition', (position, callback) => {
					this.joinApplication(device, Media).then((player) => {
				...
				speaker.on('setActive', (isActive, callback) => {
					if (isActive) {
						// this.getApplication(device, Media).then(player => setTimeout(player.disconnect, 5000));
					} else {
	...
	play(device, callback) {
		this.joinApplication(device, [Youtube, DefaultMediaReceiver, Media]).then((app) => {
			app.play((err, result) => {
				app.disconnect();
				callback(err, result);
			});
	...
	getPlaying(device, callback) {
		this.joinApplication(device, [Youtube, DefaultMediaReceiver, Media]).then((app) => {
			app.getState((err, result) => {
				app.disconnect();
				// TODO check if this works for youtube
				// TODO add updateState when chromecast is not playing something from the mediamanager (for instance an audio url)
				callback(err, result.playerState === 'PLAYING');
			});
		}).catch((err) => {
			if (err) {
				return callback(err);
