...
class Driver extends events.EventEmitter {
	...
	_onMdnsDevice(device) {
		...
		console.log('found device with txtMD', device.txtObj.md, device);
		if (!(
				device.txtObj && device.txtObj.md && device.txtObj.id && device.txtObj.fn &&
				(this._txtMd.indexOf(device.txtObj.md) !== -1 || device.type.find(type => type.name === 'googlecast'))
			)) {
			if (device.txtObj && device.txtObj.md && this._txtMdBlacklist.indexOf(device.txtObj.md) === -1) {
				if (!this._mdnsDevices[device.txtObj.id.replace(/-/g, '')]) {
					logger.captureMessage('Unknown chromecast device found', { extra: { device } });
				}
			} else {
				return;
			}
				...
				return new Promise((resolve, reject) => {
					...
					this.joinApplication(device, Application).then((app) => {
						...
						device.client.launch(Application, (err, app) => {
							...
							app.once('close', () => {
								console.trace('APPLICATION CLOSE EVENT IN DRIVER');
								if (device.apps[Application.APP_ID] === appPromise) {
									device.apps[Application.APP_ID] = null;
								}
								app.disconnect();
